...
void print_device_info(RecoveryUI* ui) {
  ...
  // I'm keeping this here for future reference. It has no use.
  ...
  std::string product_fingerprint = android::base::StringPrintf(
      "%s/%s/%s:%s/%s/%s:%s/%s", android::base::GetProperty("ro.product.brand", "").c_str(),
...
void print_credits(RecoveryUI* ui) {
  ...
  ui->Print("  Recovery Base: CrDroid Team\n");
  ui->Print("  Recovery Mods: Leah (@tromsobadet on discord)\n\n");
  return;
}
...
void clear_console(RecoveryUI* ui) {
  ...
  ui->PrintOnScreenOnly("%s", x.c_str());
  return;
}
...
std::string cmd(std::string cmd) {
  ...
  // Allocating 512mb, should be enough for everything you would want to see.
  char buffer[524288];
  std::string result = "";
  while (!feof(pipe.get())) {
    if (fgets(buffer, 524288, pipe.get()) != NULL) result += buffer;
  }
  return result;
}
...
void print_kernel_revision(RecoveryUI* ui) {
  ...
  // Uname puts a newline by itself so we don't need to add two.
  ui->Print("Kernel Version: Linux %s\n", cmd(command).c_str());
  return;
}

void read_file_to_screen(std::string path, Device* device, RecoveryUI* ui){
  std::string tmpfile = "/tmp/" + path.substr(path.find_last_of("/") + 1);
  // First check if the tmpfile already exists...
  std::ifstream infile(tmpfile);
  if(infile.good()){
    ui->Print("%s already exists, deleting...", tmpfile.c_str());
    // Terrifying  
...
void print_kernel_logs(Device* device, RecoveryUI* ui) {
  read_file_to_screen("/dev/kmsg", device, ui);
  return;
}

void print_kernel_cmdline(Device* device, RecoveryUI* ui) {
  read_file_to_screen("/proc/cmdline", device, ui);
  return;
}